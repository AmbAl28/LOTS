"use strict";
/*
 * app.js — Crooked Mirror (v3): «кривое зеркало» для лиц.
...
const MANUAL = {
  none:        {},
  wave:        { warp: 1, ampX: 16, ampY: 16, waveLenX: 130, waveLenY: 130, speed: 1 },
...
const PRESETS = {
  cheeshire:   { warp: 2, c: 'mouthL', rf: 'mw', r: 1.15, strength: 1.0,  ex: { c: 'mouthR', rf: 'mw', r: 1.15, strength: 1.0 } },
  bigmouth:    { warp: 2, c: 'mouthC', rf: 'mw', r: 3.0,  strength: 1.05 },
  minihead:    { warp: 2, c: 'faceC',  rf: 'fh', r: 1.15, strength: -0.9 },
  fatface:     { warp: 2, c: 'cheekL', rf: 'fw', r: 0.6, strength: 0.95, ex: { c: 'cheekR', rf: 'fw', r: 0.6, strength: 0.95 } },
  cathole:     { warp: 2, c: 'eyeL',   rf: 'ew', r: 0.34, strength: -1.1, ex: { c: 'eyeR', rf: 'ew', r: 0.34, strength: -1.1 } },
  bulleyes:    { warp: 2, c: 'eyeL',   rf: 'ew', r: 0.22, strength: 1.05, ex: { c: 'eyeR', rf: 'ew', r: 0.22, strength: 1.05 } },
  bloathead:   { warp: 2, c: 'faceC',  rf: 'fh', r: 0.95, strength: 0.85 },
  pinhead:     { warp: 2, c: 'faceC',  rf: 'fh', r: 1.1,  strength: -0.95, ex: { c: 'nose', rf: 'fh', r: 0.4, strength: 1.0 } },
  cathole_warp:{ warp: 7, c: 'nose',   rf: 'fh', r: 1.05, strength: 1.15, speed: 0.8, animate: 1 },
  fishlips:    { warp: 2, c: 'mouthC', rf: 'mw', r: 1.6,  strength: 1.05 },
  melty:       { warp: 9, c: 'nose',   rf: 'fh', r: 1.1,  strength: 0.75, speed: 0.5, animate: 1 },
  spooky:      { warp: 9, c: 'nose',   rf: 'fh', r: 1.25, strength: -1.1 },
  caterpillar: { warp: 1, ampX: 30, ampY: 10, waveLenX: 26, waveLenY: 130, speed: 2.4 },
  twirlface:   { warp: 7, c: 'nose',   rf: 'fh', r: 1.2,  strength: 0.9, speed: 1.1, animate: 1 },
  wobbleface:  { warp: 1, ampX: 22, ampY: 26, waveLenX: 64, waveLenY: 88, speed: 2.1 },
  bubbleface:  { warp: 2, c: 'faceC',  rf: 'fh', r: 0.9,  strength: 1.05 },
  rippleface:  { warp: 8, c: 'nose',   rf: 'fh', r: 1.0,  strength: 1.1, speed: 1.6, animate: 1 },
  flipface:    { mirror: 1 },
...
/* ---------------- параметры UI ---------------- */
const P = { maskMode: 0, expand: 0.12, feather: 12, emojiScale: 1, randInterval: 1.6, wobble: true, viz: false };
...
function emojiCanvas(ch) {
  ...
  const k = Math.min(1, (t * 0.96) / Math.max(w, fs));
...
el.fileInput.addEventListener('change', async () => {
  ...
  await ensureFaces();
  setStatus(`Файл «${f.name}» — готово, включайте эффекты из списка.`, 'ok');
});
...
el.startScreen.addEventListener('click', async () => {
  ...
  try {
    ...
    await ensureFaces();
    setStatus('Экран транслируется. Выберите эффект — лица захватываются на лету.', 'ok');
  } catch (e) {
...
el.startCam.addEventListener('click', async () => {
  ...
  try {
    ...
    if (vt) vt.addEventListener('ended', stopCurrent);
    await ensureFaces();
    setStatus('Камера включена. Выбирайте пресеты — лицо отслеживается автоматически.', 'ok');
  } catch (e) {
...
function buildFrameParams(now, w, h) {
  ...
  if (p.faceMode && S.largest && S.tracker) {
    ...
    if (rr) { p.rect = rr.rect; p.clips = rr.clip; }
  }
  return p;
}
